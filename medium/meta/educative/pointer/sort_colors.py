...
# Solution 11: Stable 3-way partition with extra buffer
def sort_colors_v11(nums):
    # Three buckets preserving relative order; concatenate.
    zeros, ones, twos = [], [], []
    for x in nums:
        if x == 0:
            zeros.append(x)
...
# Solution 12: Count sort using collections.Counter
def sort_colors_v12(nums):
    from collections import Counter
    c = Counter(nums)
    nums[:] = [0]*c.get(0, 0) + [1]*c.get(1, 0) + [2]*c.get(2, 0)
    return nums


# Solution 13: Replace using slicing in-place trick
def sort_colors_v13(nums):
    n = len(nums)
    counts = [0, 0, 0]
    for x in nums:
        counts[x] += 1
...
# Solution 14: Dutch flag with while loop instead of for
def sort_colors_v14(nums):
    lo, mid, hi = 0, 0, len(nums) - 1
    while mid <= hi:
        if nums[mid] == 0:
            nums[lo], nums[mid] = nums[mid], nums[lo]
...
# Solution 15: Two-pointer swap zeros to front, twos to back
def sort_colors_v15(nums):
    n = len(nums)
    front = 0
    back = n - 1
    i = 0
...
# Solution 16: Stable partition with separate write indices
def sort_colors_v16(nums):
    n = len(nums)
    counts = [0, 0, 0]
    for x in nums:
        counts[x] += 1
...
# Solution 17: Builtin sort with key
def sort_colors_v17(nums):
    nums.sort(key=lambda x: x)
    return nums


# Solution 18: Builtin sort (no key, since values are 0,1,2 already sortable)
def sort_colors_v18(nums):
    nums.sort()
    return nums


# Solution 19: Counting sort with itertools chain
def sort_colors_v19(nums):
    from itertools import chain, repeat
    counts = [0, 0, 0]
    for x in nums:
        counts[x] += 1
    nums[:] = list(chain(repeat(0, counts[0]), repeat(1, counts[1]), repeat(2, counts[2])))
    return nums


# Solution 20: Recursive partitioning
def sort_colors_v20(nums):
    def partition(arr, lo, hi, pivot_val):
        i = lo
    ...
    def three_way(arr, lo, hi, a, b):
        # Place all < a, then a <= x < b, then >= b
        i = partition(arr, lo, hi, a)
        j = partition(arr, i, hi, b + 1) if a <= b else i
        return i, j

    # Use simple Dutch flag with a virtual split
    lo, mid, hi = 0, 0, len(nums) - 1
    while mid <= hi:
        v = nums[mid]
        if v == 0:
...
if __name__ == "__main__":
    solutions = [
        ...
        ("V11 (3 buckets)",       sort_colors_v11),
        ("V12 (Counter)",         sort_colors_v12),
        ("V13 (slice assign)",    sort_colors_v13),
        ("V14 (while loop)",      sort_colors_v14),
        ("V15 (front/back ptr)",  sort_colors_v15),
        ("V16 (write indices)",   sort_colors_v16),
        ("V17 (key= sort)",       sort_colors_v17),
        ("V18 (builtin sort)",    sort_colors_v18),
        ("V19 (chain+repeat)",    sort_colors_v19),
        ("V20 (recursive)",       sort_colors_v20),
    ]
