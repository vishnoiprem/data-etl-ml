...
# Solution 11: Try deleting left char on mismatch, recurse
def valid_palindrome_ii_11(s):
    n = len(s)
    i, j = 0, n - 1
    ...
    if i >= j:
        return True
    # Check both skip paths with explicit helpers
    def pal(lo, hi):
        while lo < hi:
            if s[lo] != s[hi]:
                return False
...
# Solution 12: Detect first mismatch with zip
def valid_palindrome_ii_12(s):
    n = len(s)
    rev = s[::-1]
    diff_idx = -1
    for k in range(n // 2):
    ...
    if diff_idx == -1:
        return True
    # Range: s[diff_idx+1..n-1-diff_idx] or s[diff_idx..n-diff_idx-1]
    a = s[diff_idx + 1: n - 1 - diff_idx + 1]
    b = s[diff_idx: n - 1 - diff_idx]
    return a == a[::-1] or b == b[::-1]


# Solution 13: With explicit deletion flag
def valid_palindrome_ii_13(s):
    def helper(lo, hi, used):
        while lo < hi:
            if s[lo] != s[hi]:
                if used:
                    return False
...
# Solution 14: Using functools reduce on pairs
def valid_palindrome_ii_14(s):
    from functools import reduce
    n = len(s)
    # Find first non-matching pair
    def find_mismatch(idx, _):
        if idx >= n // 2:
            return -1
        if s[idx] != s[n - 1 - idx]:
            return idx
    ...
    def palindrome(t):
        return t == t[::-1]
    return (palindrome(s[:diff_idx] + s[diff_idx + 1:]) or
            palindrome(s[:n - 1 - diff_idx] + s[n - diff_idx:]))


# Solution 15: Lowercase + alnum filter (variant for full problem)
def valid_palindrome_ii_15(s):
    cleaned = ''.join(c.lower() for c in s if c.isalnum())
    n = len(cleaned)
    i, j = 0, n - 1
    while i < j and cleaned[i] == cleaned[j]:
        i += 1
        j -= 1
    if i >= j:
        return True
    return (cleaned[i + 1: j + 1] == cleaned[i + 1: j + 1][::-1] or
            cleaned[i: j] == cleaned[i: j][::-1])


# Solution 16: Greedy skip-left, verify rest is palindrome (then also try skip-right)
def valid_palindrome_ii_16(s):
    n = len(s)
    i, j = 0, n - 1
    while i < j and s[i] == s[j]:
        i += 1
    ...
    if i >= j:
        return True
    # Try skipping s[i] first; if rest palindrome, done.
    a, b = i + 1, j
...
# Solution 17: Same as V16 but try skip-right first
def valid_palindrome_ii_17(s):
    n = len(s)
    i, j = 0, n - 1
    while i < j and s[i] == s[j]:
        i += 1
...
# Solution 18: Choose which side to skip on mismatch
def valid_palindrome_ii_18(s):
    n = len(s)
    i, j = 0, n - 1
    while i < j and s[i] == s[j]:
        i += 1
    ...
    if i >= j:
        return True
    # Choose based on which has more matches
    left_matches = 0
    a, b = i + 1, j
    while a < b and s[a] == s[b]:
        left_matches += 1
    ...
    while a < b and s[a] == s[b]:
        ...
        a += 1
        b -= 1
    return left_matches == (j - i) // 2 or right_matches == (j - i) // 2


# Solution 19: Lambda-based palindrome check
def valid_palindrome_ii_19(s):
    is_pal = lambda t: t == t[::-1]
    n = len(s)
    i, j = 0, n - 1
    while i < j and s[i] == s[j]:
        i += 1
...
# Solution 20: Using a deque
def valid_palindrome_ii_20(s):
    ...
    while len(dq) > 1:
        if dq[0] != dq[-1]:
            if skipped:
                return False
            # Try popping left first; if palindrome still broken, pop right
            saved = dq.popleft()
            if dq[0] != dq[-1]:
                # Restore and pop right instead
                dq.appendleft(saved)
                dq.pop()
                if dq[0] != dq[-1]:
                    return False
...
if __name__ == "__main__":
    implementations = [
        ...
        ("Way 11: Helper is_pal", valid_palindrome_ii_11),
        ("Way 12: Detect with zip", valid_palindrome_ii_12),
        ("Way 13: Deletion flag", valid_palindrome_ii_13),
        ("Way 14: reduce on pairs", valid_palindrome_ii_14),
        ("Way 15: Clean+2ptr", valid_palindrome_ii_15),
        ("Way 16: Greedy skip-L", valid_palindrome_ii_16),
        ("Way 17: Greedy skip-R", valid_palindrome_ii_17),
        ("Way 18: Choose side", valid_palindrome_ii_18),
        ("Way 19: Lambda pal", valid_palindrome_ii_19),
        ("Way 20: Deque", valid_palindrome_ii_20),
    ]
